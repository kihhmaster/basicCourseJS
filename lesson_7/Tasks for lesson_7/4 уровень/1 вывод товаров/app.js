...
let layoutProducts = document.querySelector('.products');
...
function clickHandler(event) {
    //вам нужно очищать содержимое .products
		...
		layoutProducts.insertAdjacentHTML("afterbegin", showCategory)
		// productCategory = event.target.attributes[0].nodeValue;
		showCategory(event.target.attributes[0].nodeValue);
...
function showCategory(category) {
	...
	let editProducts = "";
	//Начиная с этого моменра решение подсмотрено у вас. Проблема заключалась в передаче значени атрибута, хотел передать через глобальную переменную productCategory  в showCategory(category);
	categoryProducts.forEach(function(products) {
		editProducts += getProductMarkup(products);
	});
	layoutProducts.insertAdjacentHTML('afterbegin', editProducts);
