let app = {
    ...
    insertPawns() {
        ...
        for (let i = 0; i < whitePawnsRow.length; i++) {
            ...
            whitePawnsRow[i].innerHTML = this.getFigure(whitePawns.figure, whitePawns.color);
        }
        ...
        for (let i = 0; i < blackPawnsRow.length; i++) {
            ...
            blackPawnsRow[i].innerHTML = this.getFigure(blackPawns.figure, blackPawns.color);
        }
    },
    ...
    generateBoard() {
        ...
        let board = '';
        // 3.1 создам переменную rowStartWithColor, в которой укажем, что линия на игровом поле будет начинаться с белого цвета
        ...
        for (let i = 0; i < 9; i++) {
            // 8.3 в innerHTML с помощью += дописываем тег td c буквой текущей колонки из this.config.cols
						if(i == 0) {
							...
							tr.innerHTML += `<td>${this.config.cols[i-1]}</td>`;
						}
