...
function hideMoreText(card) {
    ...
    let delDesc = document.querySelector('.desc');
		delDesc.remove();
...
function showMoreText(card) {
    ...
    let loremText = 'Сделайте, чтобы при клике по кнопке, вместо картинки над этой кнопкой появлялся какой-нибудь текст.';
...
// ВОПРОС?????????????????
		...
		// Не могу понять почему если вставляешь после card.productName перекращается выполнение
												// let delDesc = document.querySelector('.desc');
												// // delDesc.remove();
												// card.button.textContent = "Подробнее"; 
		...
		card.button.insertAdjacentHTML('afterend', `<div class="desc">${loremText}<div>`);
